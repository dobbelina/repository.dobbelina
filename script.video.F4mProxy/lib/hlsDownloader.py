...
class HLSDownloader:
    ...
    def keep_sending_video(
        self, dest_stream, segmentToStart=None, totalSegmentToSend=0
    ):
        try:
...
def getUrlold(url, timeout=20, returnres=False):
    ...
    try:
        ...
        openner = urllib2.build_opener(
            cookie_handler, urllib2.HTTPBasicAuthHandler(), urllib2.HTTPHandler()
        )
                ...
                if attribs["METHOD"] == "NONE":
                    ...
                    if not aesdone:
                        ...
                        assert "URI" in attribs, (
                            "EXT-X-KEY: METHOD=AES-128, but no URI found"
                        )
                        # from Crypto.Cipher import AES
                        ...
                        if gauth:
                            ...
                            if codeurl.startswith("LSDRMCallBack$"):
                                ...
                                if (
                                    ...
                                    try:
                                        callbackfilename = codeurlpath.split(
                                            os.path.sep
                                        )[-1].split(".")[0]
                                        callbackDRM = importlib.import_module(
                                            callbackfilename
                                        )
                        ...
                        if "IV" in attribs:
                            assert attribs["IV"].lower().startswith("0x"), (
                                "EXT-X-KEY: IV attribute has bad format"
                            )
                            iv = attribs["IV"][2:].zfill(32).decode("hex")
                            assert len(iv) == 16, (
                                "EXT-X-KEY: IV attribute has bad length"
                            )
                        else:
