...
site1 = AdultSite(
    "tubepornclassic",
    "[COLOR hotpink]TubePornClassic[/COLOR]",
...
def Main(url):
    ...
    if any(
        ...
        site.add_dir(
            "[COLOR hotpink]Channels[/COLOR]", siteurl + "channels", "Channels"
        )
    if any(
        x in siteurl
        for x in [
...
def List(url, page=1):
    ...
    if int(jdata.get("total_count")) - (60 * page) > 0:
        ...
        site.add_dir(
            "Next Page ({}/{})".format(str(page), str(last_page)),
            siteurl + url,
            "List",
            site.img_next,
            page,
...
def Categories(url):
    ...
    for item in sorted(jdata.get(url) or [], key=lambda x: x.get("title")):
        ...
        name = (
            name + videos
            if utils.PY3
            else (name.decode("utf-8") + videos).encode("utf-8")
        )
