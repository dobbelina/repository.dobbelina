...
def List(url):
    ...
    nextp = re.compile(r'class="pagination".+?next".+?(\d+)"', re.DOTALL | re.IGNORECASE).search(listhtml)
    if nextp:
        np = nextp.group(1)
        r = re.search(r'/\d+/', url)
        if r:
        ...
        lp = re.compile(r'class="pagination".+?last".+?(\d+)"', re.DOTALL | re.IGNORECASE).findall(listhtml)[0]
