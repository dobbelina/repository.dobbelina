...
def Playvid(url, name, download=None):
    ...
    try:
        ...
        return None
    embed = re.compile(
        "vid = '(.+?)'.+?hash = '(.+?)'", re.DOTALL | re.IGNORECASE
    ).findall(listhtml)[0]
    vid = embed[0]
    s = embed[1]
    hash = "".join(
    ...
    jsonUrl = (
        "https://www.eporner.com/xhr/video/"
        + vid
        + "?hash="
        + hash
...
def Lists(url):
    ...
    lists["HD Porn 1080p Videos - Top Rated"] = (
        site.url + "/cat/hd-1080p/SORT-top-rated/"
    )
