...
def Playvid(url, name, download=None):
    ...
    video = re.compile(r'video">.+?src="([^"]+)', re.DOTALL | re.IGNORECASE).findall(vidsite)[0]
...
def Categories(url):
    ...
    match = re.compile(r'<div\s*class="video-item">.*?a\s*href="([^"]+)">.*?<img\s*src="([^"]+)"\s*alt="([^"]+)"', re.DOTALL | re.IGNORECASE).findall(response)
