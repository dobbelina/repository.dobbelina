...
def PTMain():
    site.add_dir(
        "[COLOR hotpink]Length: [/COLOR] [COLOR orange]{0}[/COLOR]".format(ptlength),
        "",
        "PTLength",
        "",
        Folder=False,
    ...
    if not ptlogged:
        ...
        site.add_dir(
            "[COLOR hotpink]Porntrex account (favorites, subscriptions)[/COLOR]",
            "",
            "PTAccount",
            "",
        )
    ptlist = PTList(
        "{0}latest-updates/{1}".format(site.url, lengthChoices[ptlength]), 1
    )
    if not ptlist:
        utils.eod()
...
def PTAccount():
    ...
    site.add_dir(
        "[COLOR hotpink]Subscription videos[/COLOR]",
    ...
    site.add_dir(
        "[COLOR hotpink]Manage subscriptions[/COLOR]",
    ...
    site.add_dir(
        "[COLOR hotpink]Logout {0}[/COLOR]".format(ptuser),
        "",
        "PTLogin",
        "",
        Folder=False,
...
def PTList(url, page=1):
    ...
    for item in videos:
        ...
        if duration_tag:
            parts = [
                part.strip() for part in duration_tag.stripped_strings if part.strip()
            ]
            if parts:
                duration = parts[-1]
        ...
        if ptlogged:
            if "/models" in url:
                contexturl = (
                    utils.addon_sys
                    + "?mode="
                    + str("porntrex.PTSubscribe_pornstar")
                    + "&url="
                    + urllib_parse.quote_plus(url)
                )
                contextmenu.append(
                    (
                        "[COLOR deeppink]Subscribe to pornstar[/COLOR]",
            ...
            if "my_favourite_videos" in url:
                ...
                contextmenu.append(
                    (
                        "[COLOR deeppink]Delete from PT favorites[/COLOR]",
                ...
                contextmenu.append(
                    (
                        "[COLOR deeppink]Add to PT favorites[/COLOR]",
    ...
    if pagination:
        ...
        for anchor in anchors:
            ...
            if text.isdigit():
                value = int(text)
                last_numeric = (
                    value if last_numeric is None else max(last_numeric, value)
                )
        ...
        if next_href:
            ...
            next_url = (
                ...
                else site.url[:-1] + next_href
                if next_href.startswith("/")
                else urllib_parse.urljoin(url, next_href)
            )
            suffix = "/{}".format(last_numeric) if last_numeric else ""
            site.add_dir(
                "Next Page ({}{})".format(next_page, suffix),
                next_url,
                "PTList",
                site.img_next,
                next_page,
...
def PTSearch(url, keyword=None):
    if not keyword:
        ...
        searchUrl = (
            ...
            + keyword.replace(" ", "%20")
            + "/latest-updates/"
            + lengthChoices[ptlength]
            + "?mode=async&function=get_block&block_id=list_videos_videos&q="
            + keyword.replace(" ", "%20")
            + "&category_ids=&sort_by=post_date+asc&from=01"
        )
        PTList(searchUrl, 1)
...
def PTLogin(logged=True):
    ...
    if not ptlogged or "false" in ptlogged:
        ptuser = (
            utils.addon.getSetting("ptuser") if utils.addon.getSetting("ptuser") else ""
        )
        ptpass = (
            utils.addon.getSetting("ptpass") if utils.addon.getSetting("ptpass") else ""
        )
        if ptuser == "":
            ptuser = getinput(default=ptuser, heading="Input your Porntrex username")
            ptpass = getinput(
                default=ptpass, heading="Input your Porntrex password", hidden=True
            )
...
def PTSubscriptions(url, page=1):
    ...
    results = re.findall(
        'href="([^"]+)".*?data-original="([^"]+)" alt="([^"]+).+?data-id="([^"]+)',
        listhtml,
        re.DOTALL,
    )
    for url, img, name, id in results:
        ...
        if ptlogged:
            ...
            contextmenu = (
                "[COLOR deeppink]Unsubscribe[/COLOR]",
                "RunPlugin(" + contexturl + ")",
            )
        url = (
            url
            + "?mode=async&function=get_block&block_id=list_videos_common_videos_list_norm&sort_by=post_date&from4=1"
        )
    ...
    if len(results) == 11:
        ...
        suburl = suburl.replace(
            "from_my_subscriptions=" + str(page), "from_my_subscriptions=" + str(npage)
        )
...
def PTSubscribe_pornstar(url, id=None, what="subscribe"):
    ...
    suburl = "%s?mode=async&format=json&action=subscribe&subscribe_model_id=%s" % (
        url,
        id,
    )
    if what == "unsubscribe":
    ...
    if "success" in response.lower():
        ...
        if what == "unsubscribe":
            utils.notify(
                "Success", "Pornstar removed successfully from your subscriptions"
            )
            xbmc.executebuiltin("Container.Refresh")
        else:
        ...
        if what == "unsubscribe":
            utils.notify(
                "Failure", "Failure removing the pornstar from your subscriptions"
            )
        else:
...
def PTModelsAZ():
    for i in range(65, 91):
        url = "{0}/models/?mode=async&function=get_block&block_id=list_models_models_list&section={1}&sort_by=title&from=1".format(
            site.url, chr(i)
        )
...
def PTModels(url, page=1):
    ...
    results = re.findall(
        '(?si)href="([^"]+)" title="([^"]+).*?src="([^"]+)".*?videos">([^<]+)<',
        listhtml,
    )
    for modelurl, name, img, videos in results:
        url2 = (
            modelurl
            + "?mode=async&function=get_block&block_id=list_videos_common_videos_list_norm&sort_by=post_date&from=1"
        )
        if img.startswith("//"):
            img = "https:" + img
            img = img.replace(" ", "%20")
        ...
        if ptlogged and id:
            contexturl = (
                ...
                + str("porntrex.PTSubscribe_pornstar")
                + "&url="
                + urllib_parse.quote_plus(modelurl)
                + "&id="
                + str(id)
                + "&what="
                + str("subscribe")
            ...
            contextmenu = (
                "[COLOR deeppink]Subscribe[/COLOR]",
                "RunPlugin(" + contexturl + ")",
            )
        ...
        def url_constructor(self, url):
            if "categories/" in url:
                return (
                    site.url
                    + url
                    + "{}?mode=async&function=get_block&block_id=list_videos_common_videos_list_norm&from=01".format(
                        lengthChoices[ptlength]
                    )
                )
            if any(x in url for x in ["models/", "tags/"]):
                return (
                    site.url
                    + url
                    + "{}?mode=async&function=get_block&block_id=list_videos_common_videos_list_norm&sort_by=post_date&from=01".format(
                        lengthChoices[ptlength]
                    )
                )
            if "members/" in url:
