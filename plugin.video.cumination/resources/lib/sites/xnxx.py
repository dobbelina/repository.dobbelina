...
def List(url):
    ...
    for item in video_items:
        ...
        quality_tag = item.select_one(
            ".thumb-under .video-hd, .thumb-under .video-uhd, .thumb-under .video-4k, .thumb-under .video-quality, .thumb-under .video-uhd-mark"
        )
        if quality_tag:
        ...
        if not quality and metadata:
            for piece in metadata.stripped_strings:
                txt = piece.strip()
                if txt.upper() in ("HD", "4K") or (
                    txt.lower().endswith("p") and any(ch.isdigit() for ch in txt)
                ):
                    quality = txt
                    break
...
def List2(url, page=0):
    ...
    for item in items:
        ...
        quality = (
            "4K"
            if item.get("h") and item.get("fk")
            else "1440p"
            if item.get("h") and item.get("td")
            else "1080p"
            if item.get("h") and item.get("hp")
            else "720p"
            if item.get("h")
            else "360p"
            if item.get("hm") == 0
...
def Pornstars(url):
    ...
    items = re.compile(
        r'<div id="profile.+?src="([^"]+).+?href="([^"]+)">([^<]+).+?title="([^"]+)',
        re.DOTALL,
    ).findall(cathtml)
    ...
    np = re.compile(
        r'class="pagination.+?href="([^"]+)"\s*class="no', re.DOTALL | re.IGNORECASE
    ).search(cathtml)
    if np:
        currpg = re.compile(
            r'class="pagination.+?class="active".+?>(\d+)', re.DOTALL | re.IGNORECASE
        ).findall(cathtml)[0]
        lastpg = re.compile(
            r'class="pagination.+?class="last-page">(\d+)', re.DOTALL | re.IGNORECASE
        ).findall(cathtml)[0]
        pgtxt = "(Currently in Page {0} of {1})".format(currpg, lastpg)
        site.add_dir(
            "[COLOR hotpink]Next Page...[/COLOR] {0}".format(pgtxt),
            site.url[:-1] + np.group(1),
            "Pornstars",
            site.img_next,
        )
    utils.eod()
...
def Playvid(url, name, download=None):
    ...
    if "html5player.setVideoHLS" in videopage:
        videolink = re.compile(
            r"html5player.setVideoHLS\('([^']+)", re.DOTALL | re.IGNORECASE
        ).findall(videopage)[0]
