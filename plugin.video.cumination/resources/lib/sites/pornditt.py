...
def Related(url):
    contexturl = (
        utils.addon_sys + "?mode=pornditt.List&url=" + urllib_parse.quote_plus(url)
    )
    xbmc.executebuiltin("Container.Update(" + contexturl + ")")
...
def Lookupinfo(url):
    lookup_list = [
        (
            "Actor",
            r'class="btn gold" href="{}(models/[^"]+)">.+?</svg></i>([^<]+)<'.format(
                site.url
            ),
            "",
        ),
