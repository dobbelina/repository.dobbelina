...
site1 = AdultSite(
    "helloporn",
    "[COLOR hotpink]Hello Porn[/COLOR]",
    "https://hello.porn/",
    "helloporn.png",
    "helloporn",
)
...
site7 = AdultSite(
    "perfectgirls",
    "[COLOR hotpink]Perfect Girls[/COLOR]",
...
def Main(url):
    ...
    if "pornstars.tube" in url:
        site.add_dir(
            "[COLOR hotpink]Search Pornstars[/COLOR]",
            siteurl + "search/",
            "Search",
            site.img_search,
        )
    ...
    for item in video_items:
        try:
            ...
            duration_tag = item.select_one(
                '.duration_item, .fa-clock-o + span, [class*="duration"]'
            )
...
def Cat(url):
    ...
    if alphabet:
        site.add_dir(
            "[COLOR violet]Alphabet[/COLOR]", siteurl, "Letters", site.img_next
        )
...
def Related(url):
    contexturl = (
        utils.addon_sys + "?mode=" + "pornhat.List&url=" + urllib_parse.quote_plus(url)
    )
