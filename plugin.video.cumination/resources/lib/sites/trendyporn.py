...
site = AdultSite('trendyporn', "[COLOR hotpink]TrendyPorn[/COLOR]", 'https://www.trendyporn.com/', 'trendyporn.png', 'trendyporn')
...
def List(url):
    ...
    re_quality = '>HD<'
    skip = '=modelfeed'
    utils.videos_list(site, 'trendyporn.Playvid', html, delimiter, re_videopage, re_name, re_img, re_duration=re_duration, re_quality=re_quality, contextm='trendyporn.Related', skip=skip)

    re_npurl = r"href='([^']+)'\s*class=\"prevnext\">Next"
    re_npnr = r"href='page(\d+)\.html'\s*class=\"prevnext\">Next"
    utils.next_page(site, 'trendyporn.List', html, re_npurl, re_npnr, baseurl=url.split('page')[0], contextm='trendyporn.GotoPage')
    utils.eod()
...
@site.register()
def Related(url):
    contexturl = (utils.addon_sys + "?mode=" + str('trendyporn.List') + "&url=" + urllib_parse.quote_plus(url))
    xbmc.executebuiltin('Container.Update(' + contexturl + ')')
