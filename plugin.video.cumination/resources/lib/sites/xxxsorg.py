...
def List(url):
    ...
    match = re.compile(r'<div\s*class="entry-content">.*?<img src="([^"]+)".*?<a href="([^"]+)"\s*class="more-link">.+?<span\s*class="screen-reader-text">([^"]+)</span>', re.DOTALL | re.IGNORECASE).findall(html)
    ...
    nextp = re.compile('<a class="next.*?href="(.+?)">', re.DOTALL | re.IGNORECASE).search(html)
    if nextp:
