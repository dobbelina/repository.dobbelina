...
def BGList(url):
    ...
    for video in jdata['videos']:
        ...
        st = video.get('thumbs')[0].get('start')
        if st:
            e = video.get('thumbs')[0].get('end')
            p = video.get('thumbs')[0].get('pid')
            videopage = '{0}&s={1}&e={2}&p={3}'.format(videopage, st, e, p)
        name = video['title']
        if not name:
            name = video['ps_name']
        name = name + " [COLOR deeppink]" + duration + "[/COLOR]"
        name = name if utils.PY3 else name.encode('utf8')
        site.add_download_link(name, videopage, 'BGPlayvid', img, '')
