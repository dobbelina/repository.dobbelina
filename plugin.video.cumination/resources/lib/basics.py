...
def addDownLink(
    ...
    if duration:
        if addon.getSetting("duration_in_name") == "true":
            ...
            name = (
                name + duration
                if six.PY3
                else (name.decode("utf-8") + duration).encode("utf-8")
            )
        else:
            secs = 0
            try:
                ...
                if secs is None and len(duration) > 0:
                    xbmc.log(
                        "@@@@Cumination: Duration format error: " + str(duration),
                        xbmc.LOGERROR,
                    )
            except Exception:
                xbmc.log(
    ...
    if quality:
        if addon.getSetting("quality_in_name") == "true":
            ...
            name = (
                name + quality
                if six.PY3
                else (name.decode("utf-8") + quality).encode("utf-8")
            )
        else:
    ...
    if KODIVER > 19.8:
        ...
        if width:
            vtag.addVideoStream(
                xbmc.VideoStreamDetail(width=width, height=height, codec="h264")
            )
        else:
        ...
        if desc:
            liz.setInfo(
                type="Video",
                infoLabels={"Title": name, "plot": desc, "plotoutline": desc},
            )
        else:
    ...
    if fav == "del" and favorder == "date added":
        favorite_move_to_top = (
            ...
            + urllib_parse.quote_plus(name)
            + "&duration="
            + urllib_parse.quote_plus(duration)
            + "&quality="
            + urllib_parse.quote_plus(quality)
        )
        contextMenuItems.append(
            (
                "[COLOR hotpink]Move favorite to Top[/COLOR]",
                "RunPlugin(" + favorite_move_to_top + ")",
            )
        )
        favorite_move_up = (
            sys.argv[0]
        ...
        contextMenuItems.append(
            (
                "[COLOR hotpink]Move favorite Up[/COLOR]",
                "RunPlugin(" + favorite_move_up + ")",
            )
        )
        favorite_move_down = (
            sys.argv[0]
        ...
        contextMenuItems.append(
            (
                "[COLOR hotpink]Move favorite Down[/COLOR]",
                "RunPlugin(" + favorite_move_down + ")",
            )
        )
        favorite_move_to_bottom = (
            sys.argv[0]
            + "?url="
            + urllib_parse.quote_plus(url)
            + "&fav="
        ...
        contextMenuItems.append(
            (
                "[COLOR hotpink]Move favorite to Bottom[/COLOR]",
                "RunPlugin(" + favorite_move_to_bottom + ")",
            )
        )
    contextMenuItems.append(
        (
            "[COLOR hotpink]" + favtext + " favorites[/COLOR]",
            "RunPlugin(" + favorite + ")",
        )
    )
    if not noDownload:
        contextMenuItems.append(
            ("[COLOR hotpink]Download Video[/COLOR]", "RunPlugin(" + dwnld + ")")
        )
    settings_url = sys.argv[0] + "?mode=" + str("utils.openSettings")
    contextMenuItems.append(
        ("[COLOR hotpink]Addon settings[/COLOR]", "RunPlugin(" + settings_url + ")")
    )
    setview = sys.argv[0] + "?mode=" + str("utils.setview")
    contextMenuItems.append(
        (
            "[COLOR hotpink]Set this view as default[/COLOR]",
            "RunPlugin(" + setview + ")",
        )
    )
...
def get_resolution(quality):
    ...
    try:
        ...
        if len(quality) > 0 and resolution == (None, None):
            xbmc.log(
                "@@@@Cumination: Quality format error: " + str(quality), xbmc.LOGERROR
            )
    except Exception:
...
def addDir(
    ...
    if len(keyword) >= 1:
        ...
        keywcopy = (
            sys.argv[0]
            + "?mode="
            + str("utils.copySearch")
            + "&keyword="
            + urllib_parse.quote_plus(keyword)
        )
        contextMenuItems.append(
            ("[COLOR hotpink]Remove keyword[/COLOR]", "RunPlugin(" + keyw + ")")
        )
        contextMenuItems.append(
            ("[COLOR hotpink]Edit keyword[/COLOR]", "RunPlugin(" + keywedit + ")")
        )
        contextMenuItems.append(
            ("[COLOR hotpink]Copy keyword[/COLOR]", "RunPlugin(" + keywcopy + ")")
        )
    if list_avail:
        ...
        contextMenuItems.append(
            (
                "[COLOR hotpink]%s[/COLOR]" % list_item_name,
                "RunPlugin(" + list_url + ")",
            )
        )
    if listitem_id:
        move_listitem_url = (
            sys.argv[0]
            + "?mode="
            + str("favorites.move_listitem")
            + "&listitem_id="
            + str(listitem_id)
        )
        contextMenuItems.append(
            (
                "[COLOR hotpink]Move item to ...[/COLOR]",
                "RunPlugin(" + move_listitem_url + ")",
            )
        )
        listitem_url = (
            sys.argv[0]
            + "?mode="
            + str("favorites.remove_listitem")
            + "&listitem_id="
            + str(listitem_id)
        )
        contextMenuItems.append(
            (
                "[COLOR hotpink]Remove from list[/COLOR]",
                "RunPlugin(" + listitem_url + ")",
            )
        )
        moveupitem_url = (
            sys.argv[0]
            + "?mode="
            + str("favorites.moveup_listitem")
            + "&listitem_id="
            + str(listitem_id)
        )
        contextMenuItems.append(
            ("[COLOR hotpink]Move item Up[/COLOR]", "RunPlugin(" + moveupitem_url + ")")
        )
        movedownitem_url = (
            sys.argv[0]
            + "?mode="
            + str("favorites.movedown_listitem")
            + "&listitem_id="
            + str(listitem_id)
        )
        contextMenuItems.append(
            (
                "[COLOR hotpink]Move item Down[/COLOR]",
                "RunPlugin(" + movedownitem_url + ")",
            )
        )

    if custom_list:
        editlist_url = (
            sys.argv[0] + "?mode=" + str("favorites.edit_list") + "&rowid=" + str(url)
        )
        contextMenuItems.append(
            ("[COLOR hotpink]Edit name[/COLOR]", "RunPlugin(" + editlist_url + ")")
        )
        dellist_url = (
            sys.argv[0] + "?mode=" + str("favorites.remove_list") + "&rowid=" + str(url)
        )
        contextMenuItems.append(
            ("[COLOR hotpink]Remove list[/COLOR]", "RunPlugin(" + dellist_url + ")")
        )
        moveuplist_url = (
            sys.argv[0] + "?mode=" + str("favorites.moveup_list") + "&rowid=" + str(url)
        )
        contextMenuItems.append(
            ("[COLOR hotpink]Move list Up[/COLOR]", "RunPlugin(" + moveuplist_url + ")")
        )
        movedownlist_url = (
            sys.argv[0]
            + "?mode="
            + str("favorites.movedown_list")
            + "&rowid="
            + str(url)
        )
        contextMenuItems.append(
            (
                "[COLOR hotpink]Move list Down[/COLOR]",
                "RunPlugin(" + movedownlist_url + ")",
            )
        )
    ...
    liz.addContextMenuItems(contextMenuItems, replaceItems=False)
    ok = xbmcplugin.addDirectoryItem(
        handle=addon_handle, url=u, listitem=liz, isFolder=Folder
    )
    return ok
...
def searchDir(url, mode, page=None, alphabet=None):
    if not alphabet:
        addDir(
            "[COLOR hotpink]One time search[/COLOR]",
            url,
            "utils.oneSearch",
        ...
        addDir(
            "[COLOR hotpink]Add Keyword[/COLOR]",
            url,
            "utils.newSearch",
            cum_image("cum-search.png"),
            "",
        ...
        addDir(
            "[COLOR hotpink]Alphabetical[/COLOR]",
            url,
        ...
        if addon.getSetting("keywords_sorted") == "true":
            addDir(
                "[COLOR hotpink]Unsorted Keywords[/COLOR]",
                url,
                "utils.setUnsorted",
                cum_image("cum-search.png"),
                "",
            ...
            addDir(
                "[COLOR hotpink]Sorted Keywords[/COLOR]",
                url,
                "utils.setSorted",
                cum_image("cum-search.png"),
                "",
    ...
    try:
        if alphabet:
            c.execute(
                "SELECT * FROM keywords WHERE keyword LIKE ? ORDER BY keyword ASC",
                (alphabet.lower() + "%",),
            )
        else:
            if addon.getSetting("keywords_sorted") == "true":
