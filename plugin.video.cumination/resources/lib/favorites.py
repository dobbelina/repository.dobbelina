...
def List():
    ...
    try:
        if basics.addon.getSetting('custom_sites') == 'true':
            c.execute("SELECT f.* FROM favorites f LEFT JOIN custom_sites cs on 'custom_' || cs.name || '_by_' || cs.author = substr(f.mode, 1, instr(f.mode, '.') - 1) WHERE IFNULL(cs.enabled, 1) = 1")
        else:
            c.execute("SELECT f.* FROM favorites f LEFT JOIN custom_sites cs on 'custom_' || cs.name || '_by_' || cs.author = substr(f.mode, 1, instr(f.mode, '.') - 1) WHERE cs.name IS NULL")
...
def get_custom_listitems():
    ...
    conn.text_factory = str
    c = conn.cursor()
    c.execute("SELECT name FROM custom_listitems")
    rows = c.fetchall()
    conn.close()
    return rows
