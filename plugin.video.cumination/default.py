...
def site_list():
    ...
    for x in sorted(AdultSite.get_sites(), key=lambda y: y.get_clean_title().lower(), reverse=False):
        if x.custom:
            utils.kodilog('{0}: {1}'.format(utils.i8n('list_custom'), x.title))
        if (x.title,) in custom_listitems:
            x.title = x.title.replace('hotpink', 'violet')
        url_dispatcher.add_dir(x.title, x.url, x.default_mode, x.image, about=x.about, custom=x.custom)
    utils.eod(basics.addon_handle, False)
