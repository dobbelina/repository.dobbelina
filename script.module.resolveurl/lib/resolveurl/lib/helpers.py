...
def get_hidden(html, form_id=None, index=None, include_submit=True):
    hidden = {}
    if form_id:
        pattern = r"""<form [^>]*(?:id|name)\s*=\s*['"]?%s['"]?[^>]*>(.*?)</form>""" % (
            form_id
        )
    else:
        pattern = """<form[^>]*>(.*?)</form>"""
    ...
    for i, form in enumerate(re.finditer(pattern, html, re.DOTALL | re.I)):
        ...
        if index is None or i == index:
            for field in re.finditer(
                """<input [^>]*type=['"]?hidden['"]?[^>]*>""", form.group(1)
            ):
            ...
            if include_submit:
                match = re.search(
                    """<input [^>]*type=['"]?submit['"]?[^>]*>""", form.group(1)
                )
                if match:
...
def append_headers(headers):
    return "|%s" % "&".join(
        ["%s=%s" % (key, urllib_parse.quote_plus(headers[key])) for key in headers]
    )
    ...
    if len(sources) > 1:
        try:
            ...
            common.logger.log_debug(
                r'Scrape sources sort failed |int(re.sub("\D", "", x[0])|'
            )
            try:
                sources.sort(key=lambda x: re.sub("[^a-zA-Z]", "", x[0].lower()))
            except:
                common.logger.log_debug(
                    'Scrape sources sort failed |re.sub("[^a-zA-Z]", "", x[0].lower())|'
                )
    return sources
...
def parse_html5_source_list(html):
    label_attrib = (
        "type"
        if not re.search(r"""<source\s+src\s*=.*?data-res\s*=.*?/\s*>""", html)
        else "data-res"
    )
    sources = [
        (match[1], match[0].replace(r"\/", "/"))
...
def parse_smil_source_list(smil):
    ...
    for i in re.finditer(
        r'src\s*=\s*"([^"]+)(?:"\s*(?:width|height)\s*=\s*"([^"]+))?', smil
    ):
        label = "Unknown"
...
def scrape_sources(
    ...
    def __parse_to_list(_html, regex):
        ...
        for r in re.finditer(regex, _html, re.DOTALL):
            ...
            file_name = (
                urllib_parse.urlparse(stream_url[:-1]).path.split("/")[-1]
                if stream_url.endswith("/")
                else urllib_parse.urlparse(stream_url).path.split("/")[-1]
            )
            ...
            blocked = (
                not file_name
                or any(item in file_name.lower() for item in _blacklist)
                or any(item in label for item in _blacklist)
            )
            if (
                "://" not in stream_url
                or blocked
                or (stream_url in streams)
                or any(stream_url == t[1] for t in source_list)
            ):
                continue
            labels.append(label)
    ...
    if generic_patterns or not patterns:
        source_list += __parse_to_list(
            html,
            r"""["']?label\s*["']?\s*[:=]\s*["']?(?P<label>[^"',]+)["']?(?:[^}\]]+)["']?\s*file\s*["']?\s*[:=,]?\s*["'](?P<url>[^"']+)""",
        )
        source_list += __parse_to_list(
            html,
            r"""["']?\s*(?:file|src)\s*["']?\s*[:=,]?\s*["'](?P<url>[^"']+)(?:[^}>\]]+)["']?\s*label\s*["']?\s*[:=]\s*["']?(?P<label>[^"',]+)""",
        )
        source_list += __parse_to_list(
            html, r"""video[^><]+src\s*[=:]\s*['"](?P<url>[^'"]+)"""
        )
        source_list += __parse_to_list(
            html,
            r"""source\s+src\s*=\s*['"](?P<url>[^'"]+)['"](?:.*?res\s*=\s*['"](?P<label>[^'"]+))?""",
        )
        source_list += __parse_to_list(
            html, r"""["'](?:file|url)["']\s*[:=]\s*["'](?P<url>[^"']+)"""
        )
        source_list += __parse_to_list(
            html, r"""param\s+name\s*=\s*"src"\s*value\s*=\s*"(?P<url>[^"]+)"""
        )
...
def scrape_subtitles(
    html, rurl="", scheme="http", patterns=None, generic_patterns=True
):
    if patterns is None:
        patterns = []
    ...
    def __parse_to_dict(_html, regex):
        ...
        for r in re.finditer(regex, _html, re.DOTALL):
            ...
            file_name = (
                urllib_parse.urlparse(subs_url[:-1]).path.split("/")[-1]
                if subs_url.endswith("/")
                else urllib_parse.urlparse(subs_url).path.split("/")[-1]
            )
            label = match.get("label", file_name)
            if label is None:
                label = file_name
    ...
    if generic_patterns or not patterns:
        subtitles.update(
            __parse_to_dict(
                html,
                r"""{\s*file:\s*["'](?P<url>[^"']+)["'],\s*label:\s*["'](?P<label>[^"']+)["'],\s*kind:\s*["'](?:captions|subtitles)["']""",
            )
        )
        subtitles.update(
            __parse_to_dict(
                html,
                r"""<track\s*kind=['"]?(?:captions|subtitles)['"]?\s*src=['"](?P<url>[^'"]+)['"]\s*srclang=['"](?P<label>[^'"]+)""",
            )
        )
        subtitles.update(
            __parse_to_dict(
                html,
                r"""<track\s*kind="(?:captions|subtitles)"\s*label="(?P<label>[^"]+)"\s*srclang="[^"]+"\s*src="(?P<url>[^"]+)""",
            )
        )
        subtitles.update(
            __parse_to_dict(
                html,
                r""""tracks":.+?"kind":\s*"captions",\s*"file":\s*"(?P<url>[^"]+).+?"label":\s*"(?P<label>[^"]+)""",
            )
        )
        subtitles.update(
            __parse_to_dict(
                html,
                r'''"file":\s*"(?P<url>[^"]+).+?"label":\s*"(?P<label>[^"]+)","kind":"(?:captions|subtitles)"''',
            )
        )
...
def get_media_url(
    ...
    result_blacklist = list(
        set(result_blacklist + [".smil"])
    )  # smil(not playable) contains potential sources, only blacklist when called from here
    ...
    source_list = scrape_sources(
        html, result_blacklist, scheme, patterns, generic_patterns, rurl
    )
    source = pick_source(source_list)
    source = urllib_parse.quote(source, "/:?=&!") + append_headers(headers)
    if subs:
        subtitles = scrape_subtitles(
            html, rurl, scheme, subs_patterns, generic_subs_patterns
        )
...
def cleanse_html(html):
    ...
    html = re.sub(
        r"""<(div|span)[^>]+style=["'](visibility:\s*hidden|display:\s*none);?["']>.*?</\\1>""",
        "",
        html,
        re.I | re.DOTALL,
    )
...
def girc(page_data, url, co=None):
    ...
    """
    net = common.Net()
    hdrs = {"User-Agent": common.FF_USER_AGENT, "Referer": url}
    rurl = "https://www.google.com/recaptcha/api.js"
    aurl = "https://www.google.com/recaptcha/api2"
    key = re.search(
        r'(?:src="{0}\?.*?render|data-sitekey)="?([^"]+)'.format(rurl), page_data
    )
    if key:
        if co is None:
        ...
        page_data2 = net.http_GET(
            "{0}/anchor?{1}".format(aurl, urllib_parse.urlencode(rdata)), headers=hdrs
        ).content
        ...
        if rtoken:
            ...
            return ""
        pdata = {"v": v, "reason": "q", "k": key, "c": rtoken, "sa": "", "co": co}
        hdrs.update({"Referer": aurl})
        page_data3 = net.http_POST(
            "{0}/reload?k={1}".format(aurl, key), form_data=pdata, headers=hdrs
        ).content
...
def arc4(t, n):
    ...
    for c in range(len(n)):
        ...
        s[e], s[u] = s[u], s[e]
        h += chr(
            (n[c] if isinstance(n[c], int) else ord(n[c])) ^ s[(s[e] + s[u]) % 256]
        )
    return h
...
def tear_decode(data_file, data_seed):
    ...
    def tea_code(a79, a80):
        ...
        for a86 in range(32):
            a85 += i32(
                (((i32(a83).value << 4) ^ rshift(i32(a83).value, 5)) + a83)
                ^ (a87 + a80[(a87 & 3)])
            ).value
            a85 = i32(a85 | 0).value
            a87 = i32(a87).value - i32(1640531527).value
            a83 += i32(
                (((i32(a85).value << 4) ^ rshift(i32(a85).value, 5)) + a85)
                ^ (a87 + a80[(rshift(a87, 11) & 3)])
            ).value
            a83 = i32(a83 | 0).value
        return [a85, a83]
    ...
    def tea_decode(a90, a91):
        ...
        for a98 in range(32):
            a96 = i32(a96).value - (
                (((i32(a95).value << 4) ^ rshift(i32(a95).value, 5)) + a95)
            ...
            a95 = (
                i32(a95).value
                - i32(
                    (((i32(a96).value << 4) ^ rshift(i32(a96).value, 5)) + a96)
                    ^ (a97 + a91[(a97 & 3)])
                ).value
            )
            a95 = i32(a95 | 0).value
